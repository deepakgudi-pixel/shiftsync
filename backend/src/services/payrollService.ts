...
const fetchAllShiftsForPeriod = async (orgId, startDate, endDate, client = null) => {
...
const findPayPeriodById = async (id, orgId, client = null) => {
  const runner = client || { query: (text, params) => query(text, params) };
  const result = await runner.query(
...
const findActiveOvertimeRule = async (orgId, client = null) => {
  const runner = client || { query: (text, params) => query(text, params) };
  const result = await runner.query(
...
const findEmployeesWithRates = async (orgId, endDate, client = null) => {
  const runner = client || { query: (text, params) => query(text, params) };
  const result = await runner.query(
...
const insertPayrollSnapshot = async (client, data) => {
  await client.query(
    ...
    [
      ...
      JSON.stringify(data.rulesSnapshot || {}),
...
const processPayPeriod = async (payPeriodId, orgId, memberId, clerkUserId, req) => {
  ...
  try {
    ...
    if (!period) {
      await client.query("ROLLBACK");
      return { status: 404, body: { error: "Pay period not found" } };
    }
    if (period.status !== "DRAFT") {
      await client.query("ROLLBACK");
      return { status: 400, body: { error: "Period already processed" } };
    }
    ...
    for (const emp of employees) {
      ...
      generated.push({ id: payslip.id, memberId: emp.id, name: emp.name, totalEarn: payroll.totalEarnings });
    }
    ...
    const fanoutJob = await enqueueJob({
      client,
      organisationId: orgId,
    ...
    await client.query("COMMIT");
    await publishQueuedJob(fanoutJob);
