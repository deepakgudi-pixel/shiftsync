...
const { orgRoom, shiftRoom } = require("../lib/socketRooms");
...
router.post("/clock-in", requireAuth, async (req, res) => {
  try {
    ...
    req.io.to(orgRoom(req.member.organisation_id)).emit("attendance:clockIn", clockInPayload);
    req.io.to(shiftRoom(shiftId)).emit("attendance:clockIn", clockInPayload);
...
router.post("/clock-out", requireAuth, async (req, res) => {
  try {
    ...
    req.io.to(orgRoom(req.member.organisation_id)).emit("attendance:clockOut", clockOutPayload);
    req.io.to(shiftRoom(shiftId)).emit("attendance:clockOut", clockOutPayload);
...
const areDebugEndpointsEnabled = () => (
  process.env.NODE_ENV !== "production" &&
  process.env.ATTENDANCE_DEBUG_ENDPOINTS_ENABLED === "true"
);

// Debug endpoints — development / staging only, behind an explicit feature flag
