...
router.put("/:id", requireAuth, requireRole("ADMIN", "MANAGER"), [
  ...
  try {
    ...
    const updated = await shiftService.updateShift(req.params.id, req.member.organisation_id, {
      ...
      assigneeDefined, assigneeId: finalAssigneeId, status,
    });
...
router.delete("/:id", requireAuth, requireRole("ADMIN", "MANAGER"), async (req, res) => {
  try {
    ...
    if (existing.assignee_id) {
      ...
      req.io.to(`user:${existing.assignee_id}`).emit("notification", { type: "SHIFT_CANCELLED", shiftId: existing.id });
    }
...
router.post("/:id/swap", requireAuth, [
  ...
  try {
    ...
    req.io.to(`org:${req.member.organisation_id}`).emit("swap:requested", details);
    await emitEvent({
      organisationId: req.member.organisation_id,
      memberId: req.member.id,
...
router.patch("/:id/swap/:swapId", requireAuth, requireRole("ADMIN", "MANAGER"), [
  ...
  try {
    ...
    if (status === "APPROVED") {
      ...
      req.io.to(`org:${req.member.organisation_id}`).emit("shift:updated", updatedShift);
      await emitEvent({
        organisationId: req.member.organisation_id,
        memberId: req.member.id,
