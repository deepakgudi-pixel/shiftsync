...
const setup = async () => {
  ...
  try {
    ...
    `);
    await client.query(`
      ALTER TABLE payroll_snapshots ADD COLUMN IF NOT EXISTS rules_snapshot JSONB NOT NULL DEFAULT '{}';
    `);
    await client.query(`
      ...
      CREATE INDEX IF NOT EXISTS idx_background_jobs_ready ON background_jobs(status, run_after);
      CREATE INDEX IF NOT EXISTS idx_background_jobs_org ON background_jobs(organisation_id, created_at DESC);
    ...
    await client.query(`
      ...
      BEGIN
        ...
        DELETE FROM background_jobs WHERE organisation_id = org_id;
